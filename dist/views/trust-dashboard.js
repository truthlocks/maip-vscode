...
exports.TrustDashboardPanel = void 0;
const vscode = __importStar(require("vscode"));
const crypto = __importStar(require("crypto"));
class TrustDashboardPanel {
    static currentPanel;
    panel;
    disposables = [];
    constructor(panel, _extensionUri, trustScore, history) {
        this.panel = panel;
        this.panel.onDidDispose(() => this.dispose(), null, this.disposables);
        this.panel.webview.html = this.getHtml(trustScore, history);
    }
    static createOrShow(extensionUri, trustScore, history) {
        ...
        const panel = vscode.window.createWebviewPanel("maip.trustDashboard", `Trust: ${trustScore.agent_id.substring(0, 8)}`, column ?? vscode.ViewColumn.One, {
            enableScripts: false,
            retainContextWhenHidden: true,
        });
        TrustDashboardPanel.currentPanel = new TrustDashboardPanel(panel, extensionUri, trustScore, history);
    }
    dispose() {
    ...
    getHtml(trustScore, history) {
        const nonce = crypto.randomBytes(16).toString("hex");
        const score = trustScore.trust_score;
        const scoreColor = score >= 0.8 ? "#4ec94e" : score >= 0.5 ? "#e6b422" : "#c94e4e";
...
exports.TrustDashboardPanel = TrustDashboardPanel;
...
function renderHistoryChart(history) {
    ...
    const sorted = [...history].sort((a, b) => new Date(a.computed_at).getTime() - new Date(b.computed_at).getTime());
    ...
    for (let val = 0; val <= 1; val += 0.25) {
        ...
        gridLines.push(`<line x1="${padding.left}" y1="${y.toFixed(1)}" x2="${width - padding.right}" y2="${y.toFixed(1)}" stroke="var(--vscode-panel-border)" stroke-width="0.5"/>`, `<text x="${padding.left - 5}" y="${(y + 4).toFixed(1)}" text-anchor="end" fill="var(--vscode-descriptionForeground)" font-size="9">${(val * 100).toFixed(0)}</text>`);
    }
    ...
    ${sorted
        ...
        const x = padding.left + (i / Math.max(sorted.length - 1, 1)) * chartW;
        const y = padding.top + (1 - entry.trust_score) * chartH;
        const color = entry.trust_score >= 0.8 ? "#4ec94e" : entry.trust_score >= 0.5 ? "#e6b422" : "#c94e4e";
        return `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="3" fill="${color}"/>`;
    })
        .join("\n    ")}
  </svg>`;
}
function renderComponents(trustScore) {
    const components = [
        ...
        { label: "Delegation Depth", value: trustScore.score_components.delegation_depth },
        { label: "Verification History", value: trustScore.score_components.verification_history },
        { label: "Multi-Witness", value: trustScore.score_components.multi_witness },
        { label: "Anomaly Penalty", value: trustScore.score_components.anomaly_penalty },
    ];
    return components
        .map((c) => {
        const pct = Math.max(0, Math.min(1, c.value)) * 100;
        const color = c.label === "Anomaly Penalty"
            ? (c.value > 0 ? "#c94e4e" : "#4ec94e")
            : (c.value >= 0.7 ? "#4ec94e" : c.value >= 0.4 ? "#e6b422" : "#c94e4e");
...
//# sourceMappingURL=trust-dashboard.js.map