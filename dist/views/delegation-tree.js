...
exports.DelegationTreePanel = void 0;
const vscode = __importStar(require("vscode"));
const crypto = __importStar(require("crypto"));
const config_1 = require("../config");
class DelegationTreePanel {
    static currentPanel;
    panel;
    disposables = [];
    constructor(panel, delegations, rootAgentId) {
        this.panel = panel;
        this.panel.onDidDispose(() => this.dispose(), null, this.disposables);
        this.panel.webview.html = this.getHtml(delegations, rootAgentId);
    }
    static async createOrShow(_extensionUri, client) {
        ...
        try {
            ...
            const panel = vscode.window.createWebviewPanel("maip.delegationTree", "MAIP Delegation Tree", column ?? vscode.ViewColumn.One, {
                enableScripts: false,
    ...
    getHtml(delegations, rootAgentId) {
...
exports.DelegationTreePanel = DelegationTreePanel;
function buildTree(delegations, rootAgentId) {
    ...
    function buildNode(agentId, depth) {
        const children = (childMap.get(agentId) ?? []).map((d) => buildNode(d.child_agent_id, depth + 1));
        return {
            agentId,
...
function renderTreeSvg(root) {
    ...
    function renderNode(pNode) {
        ...
        for (const child of pNode.children) {
            ...
            svgElements.push(`<line x1="${cx}" y1="${pNode.y + nodeHeight}" x2="${childCx}" y2="${childCy}" stroke="var(--vscode-panel-border)" stroke-width="1.5"/>`);
        }
        ...
        svgElements.push(`<rect x="${pNode.x}" y="${pNode.y}" width="${nodeWidth}" height="${nodeHeight}" rx="4" fill="${fill}" stroke="${strokeColor}" stroke-width="1.5"/>`, `<text x="${cx}" y="${cy + 4}" text-anchor="middle" fill="var(--vscode-foreground)" font-size="11">${escapeHtml(pNode.node.displayName)}...</text>`);
        for (const child of pNode.children) {
            renderNode(child);
        }
    }
...
function renderDelegationTable(delegations) {
